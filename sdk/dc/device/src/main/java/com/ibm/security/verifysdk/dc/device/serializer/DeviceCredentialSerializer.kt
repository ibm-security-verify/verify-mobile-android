...
package com.ibm.security.verifysdk.dc.device.serializer
...
import com.ibm.security.verifysdk.dc.device.model.MDocCredential
import com.ibm.security.verifysdk.dc.device.model.SDJwtCredential
...
object DeviceCredentialSerializer : KSerializer<DeviceCredentialDescriptor> {
    ...
    /**
     * Deserializes a [DeviceCredentialDescriptor] from JSON.
     *
     ...
     * @since 3.0.7
     */
    override fun deserialize(decoder: Decoder): DeviceCredentialDescriptor {
        ...
        return when (format) {
            CredentialFormat.SDJWT -> SDJwtCredential.from(jsonObject)
     ...
     * @since 3.0.7
     */
    override fun serialize(encoder: Encoder, value: DeviceCredentialDescriptor) {
        require(encoder is JsonEncoder)

        val jsonObject = when (value) {
            is SDJwtCredential -> value.toJsonObject(json)
            is MDocCredential -> value.toJsonObject(json)
            else -> throw NotImplementedError("Unsupported DeviceCredentialDescriptor type")
        }
